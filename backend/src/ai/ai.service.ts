...
export class AiService {
  ...
  async triagePatient(input: TriageInput): Promise<TriageResult> {
    ...
    try {
      ...
      this.logger.warn(
        `AI triage failed, falling back to offline: ${(error as Error).message}`,
      );
      return this.triageOffline(input);
    }
  }

  private async triageWithAI(input: TriageInput): Promise<TriageResult> {
  ...
  private triageOffline(input: TriageInput): TriageResult {
    const severityScores: Record<string, number> = {
      severe: 9,
      moderate: 6,
      mild: 3,
    };
    const symptomAlerts: Record<string, number> = {
    ...
    for (const symptom of input.symptoms || []) {
      ...
      for (const [alert, score] of Object.entries(symptomAlerts)) {
        if (lower.includes(alert)) urgencyScore = Math.max(urgencyScore, score);
      }
    }
...
Réponds UNIQUEMENT en JSON: { "priority": "low|medium|high|emergency", "recommendation": "...", "nextSteps": [...], "urgencyLevel": 1-10 }`;
  ...
  async generateResponse(prompt: string): Promise<string> {
    ...
    try {
      ...
      this.logger.error(`AI response failed: ${(error as Error).message}`);
      return this.generateOfflineResponse(prompt);
    }
  }
